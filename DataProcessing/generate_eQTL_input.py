...
def Preprocess_CNV_Genotype(Exp_matrix_file,Gene_CNA_file, CNV_Genotype_file,eQTL_Exp_file,eQTL_Genotype_file):
    ...
    df_Gene_CNV = pd.read_table(Gene_CNA_file, sep="\t", index_col=0)
    
    common_donor_id = [donor_id for donor_id in df_Exp if (donor_id in df_CNV_Genotype and donor_id in df_Gene_CNV)]
    df_CNV_Genotype = df_CNV_Genotype[common_donor_id]
    df_Exp = df_Exp[common_donor_id]
    df_Gene_CNV = df_Gene_CNV[common_donor_id]

    df_Exp = df_Exp.T; df_Gene_CNV = df_Gene_CNV.T
    common_gene_symbol = [gene_symbol for gene_symbol in df_Gene_CNV if gene_symbol in df_Exp]
    df_Exp = df_Exp[common_gene_symbol]
    df_Gene_CNV = df_Gene_CNV[common_gene_symbol]
    df_Exp = df_Exp.T; df_Gene_CNV = df_Gene_CNV.T
    print "----Number of donor: Gene_Exp (%d), Gene_copynumber (%d), CNV_genotype (%d)" % (len(df_Exp.columns), len(df_Gene_CNV.columns), len(df_CNV_Genotype.columns))
    print "----Number of Genes: Gene_copynumber (%d), Gene_Exp (%d)" % (len(df_Gene_CNV), len(df_Exp))
    ...
    df_Exp = df_Exp.T
    df_Exp = df_Exp[[gene_symbol for gene_symbol in df_Exp if [df_Exp[gene_symbol]>4][0].tolist().count(True) <3]]
    df_Exp = df_Exp.T
    ...
    CNV_Genotype_matrix = [[0 if df_CNV_Genotype[donor_id][gene_symbol]==0 else 1 for donor_id in df_CNV_Genotype.columns] for gene_symbol in df_CNV_Genotype.index]
    ...
    print "There are %d donor in Expression file" % len(df_Exp_eQTL.columns)
    print "There are %d genes in Expression file" % len(df_Exp_eQTL)
    print "There are %d donor in Geontype file" % len(df_Genotype_eQTL.columns)
    print "There are %d genotypes in Genotype file" % len(df_Genotype_eQTL)
    return True
def Preprocess_SV_Genotype(Exp_matrix_file,Gene_CNA_file, SV_Genotype_file,SV_eQTL_Exp_file,SV_eQTL_Genotype_file):
    ...
    common_donor_id = [donor_id for donor_id in df_Exp if (donor_id in df_SV_Genotype and donor_id in df_Gene_CNV)]
    df_SV_Genotype = df_SV_Genotype[common_donor_id]
    df_Exp = df_Exp[common_donor_id]
    df_Gene_CNV = df_Gene_CNV[common_donor_id]

    df_Exp = df_Exp.T; df_Gene_CNV = df_Gene_CNV.T
    common_gene_symbol = [gene_symbol for gene_symbol in df_Exp if gene_symbol in df_Gene_CNV]
    df_Exp = df_Exp[common_gene_symbol]
    df_Gene_CNV = df_Gene_CNV[common_gene_symbol]
    df_Exp = df_Exp.T; df_Gene_CNV = df_Gene_CNV.T
    print "----Number of donor: Gene_Exp (%d), Gene_copynumber (%d), CNV_genotype (%d)" % (len(df_Exp.columns), len(df_Gene_CNV.columns), len(df_SV_Genotype.columns))
    ...
    SV_Genotype_matrix = [[0 if df_SV_Genotype[donor_id][gene_symbol]==0 else 1 for donor_id in df_SV_Genotype.columns] for gene_symbol in df_SV_Genotype.index]
...
def main():
    ...
    Gene_CNA_file = "../Temp_file/matrix_file/OV-AU.cnv_matrix.tsv"
    ...
    # Preprocess_CNV_Genotype(Exp_matrix_file,Gene_CNA_file, CNV_Genotype_file,eQTL_Exp_file,eQTL_Genotype_file)
